{
  ...
  mountOptions = cfg.lvm-luks-btrfs.btrfsOptions;
...
in {
  options.${namespace}.storage.lvm-luks-btrfs = {
      ...
      lvm_vg.pool = {
        ...
        lvs = {
          main = {
            ...
            content = let
              homes = cfg.lvm-luks-btrfs.homes;
              subvolumes = lib.mkMerge (lib.lists.flatten ([
                  (map mkSubvolumeWithSnapshots cfg.lvm-luks-btrfs.subvolumesWithSnapshots)
                  (map mkSubvolume cfg.lvm-luks-btrfs.subvolumes)
                ]
          ...
          system = {
            ...
            content.type = "btrfs";
            content.subvolumes = lib.mkMerge (lib.mapAttrsToList mkNoDataCow cfg.lvm-luks-btrfs.noDataCow);
          };
          swap = {
            size = lib.mkDefault "16G";
            content.type = "swap";
          };
        };
      };
